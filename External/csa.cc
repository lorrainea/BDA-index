...
pair<INT,INT> pattern_matching ( string & w, string & a, csa_sada<enc_vector<>, 32, 32, sa_order_sa_sampling<>, int_vector<>, byte_alphabet> SA, INT * LCP,rmq_succinct_sct<> &rmq, INT n )
{
	...
	for(auto &pattern : new_all_pat)
   	{
		...
		pair<INT,INT> interval = pattern_matching ( pattern, text_string, csa1, LCP, rmq, text_string.size() );
