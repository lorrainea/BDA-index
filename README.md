...
<b>Which index shall I use?</b> BDA-index_II is considerably faster in practice than BDA-index_I, in particular when the number of occurrences is high; and it is also smaller.
BDA-index_I has provably near-optimal queries but relies on 2D range reporting for the queries, which can be very slow in practice.
